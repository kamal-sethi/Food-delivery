  ...
  return (
    ...
    >
      ...
      {showLeft && (
        <button
          onClick={() => scroll("left")}
          className="cursor-pointer absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg
      ...
      <div
        className="flex gap-6 overflow-x-auto px-10 -b-4 scrollbar-hide scroll-smooth"
        ref={scrollRef}
      >
        {categories.map((cat) => {
          const Icon = cat.icon;
      ...
      {showRight && (
        <button
          ...
          className="cursor-pointer absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg
      hover:bg-green-100 rounded-full w-10 h-10 flex items-center justify-center transition-all"
        >
