...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    increaseQuantity: (
      ...
      const item = state.cartData.find((c) => c._id == action.payload);
      if (item) {
    ...
    decreaseQuantity: (
      ...
      if (item?.quantity && item?.quantity > 1) {
        ...
        state.cartData = state.cartData.filter((c) => c._id != item?._id);
      }
      cartSlice.caseReducers.calculateTotal(state);
    },
    deleteItem: (state, action: PayloadAction<mongoose.Types.ObjectId>) => {
